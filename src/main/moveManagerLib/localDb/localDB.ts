...
import simpleGit, { CleanOptions, SimpleGit, LogResult } from "simple-git";
...
export class LocalDb {
  ...
  public async getSet(
    setId: string
  ): Promise<SetWithCompleteMetadata | undefined> {
        ...
        if (path.extname(metaFile) === ".json") {
          ...
          if (fs.existsSync(setPath)) {
            ...
            try {
              ...
              console.error(
                `Error reading or parsing metadata file ${metaPath}:`,
                err
              );
  ...
  public async getSetVersionFromRevision(
    setId: string,
    revision: string
  ): Promise<string> {
  ...
  private async storeMetadata(completeSetMetadata: CompleteSetMetadata) {
    const metaPath = path.join(
      this.metaDirPath,
      `${completeSetMetadata.setId}.json`
    );
    await fs.promises.writeFile(
      metaPath,
      JSON.stringify(completeSetMetadata, null, 2)
    );
  }
}
