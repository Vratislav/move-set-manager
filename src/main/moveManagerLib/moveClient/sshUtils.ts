...
export async function getExtendedAttributes(
  ...
  if (result.code === 1) {
    throw new Error(
      `Failed to get extended attributes for ${path}: ${result.stderr}`
    );
  }
...
export async function getPathStat(
  sftp: SFTPWrapper,
  path: string
): Promise<Stats> {
    ...
    await new Promise<void>((resolve, reject) => {
      sftp.fastGet(remoteFileOrDir, localPath, (err) => {
        if (err) {
          reject(
            new Error(
              `Failed to copy file ${remoteFileOrDir} to ${localPath}: ${err.message}`
            )
          );
        } else {
          resolve();
    ...
    await new Promise<void>((resolve, reject) => {
      sftp.fastPut(localFileOrDir, remotePath, (err) => {
        if (err) {
          reject(
            new Error(
              `Failed to copy file ${localFileOrDir} to ${remotePath}: ${err.message}`
            )
          );
        } else {
          resolve();
      ...
      sftp.mkdir(remotePath, (err) => {
        if (err) {
          ...
          if (err.message !== "Failure") {
            return reject(
              new Error(
                `Failed to create remote directory ${remotePath}: ${err.message}`
              )
            );
          }
        }
...
export async function renameRemoteDirectoryOrFile(
  ...
  await new Promise<void>((resolve, reject) => {
    sftp.rename(oldPath, newPath, (err) => {
      if (err) {
        reject(
          new Error(`Failed to rename ${oldPath} to ${newPath}: ${err.message}`)
        );
      } else {
        resolve();
      }
