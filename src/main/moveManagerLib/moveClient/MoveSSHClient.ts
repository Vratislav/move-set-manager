...
import { MoveSetMetadata, Set } from "../model/set";
...
export interface IMoveSSHClient {
  ...
  downloadSet(setId: string, targetDir: string): Promise<Set>;
  uploadSet(set: Set): Promise<Set>;
...
export class MoveSSHClient implements IMoveSSHClient {
  ...
  private async getSetMetadata(setId: string): Promise<MoveSetMetadata> {
    ...
    const metaData: MoveSetMetadata = {
      ...
      wasExternallyModified:
        attributes[XATTR_WAS_EXTERNALLY_MODIFIED] === "true",
    };
    return metaData;
  }
  ...
  async listSets(): Promise<Set[]> {
    ...
    const setDirStats = setsInDirStats.filter((stat) =>
      stat.attrs.isDirectory()
    );

    const sets: Set[] = []; // Initialize empty array to store results
    let setsCount = 0;
  ...
  private async getExtendedAttributes(
    path: string
  ): Promise<{ [key: string]: string }> {
    return getExtendedAttributes(this.ssh, path);
  }

  private async setExtendedAttribute(
    path: string,
    key: string,
    value: string
  ): Promise<void> {
  ...
  private async renameRemoteDirectoryOrFile(
    oldPath: string,
    newPath: string
  ): Promise<void> {
    if (!this.sftp) {
  ...
  async downloadSet(setId: string, targetDir: string): Promise<Set> {
    if (!this.sftp) {
      throw new Error("SFTP client not initialized");
    }
  ...
  async uploadSet(set: Set): Promise<Set> {
    if (!this.sftp) {
      throw new Error("SFTP client not initialized");
    }
    ...
    await this.renameRemoteDirectoryOrFile(
      remoteInternalSetPath,
      remoteTargetNamePath
    );
    ...
    await this.setExtendedAttribute(
      remoteSetIdDirPath,
      XATTR_SONG_INDEX,
      set.meta.index.toString()
    );
    await this.setExtendedAttribute(
      remoteSetIdDirPath,
      XATTR_SONG_COLOR,
      set.meta.color.toString()
    );
